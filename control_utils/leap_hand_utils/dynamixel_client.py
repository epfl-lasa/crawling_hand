...
class DynamixelClient:
    ...
    def write_vel_profile(self, motor_ids: Sequence[int],
                          vel: np.ndarray):
        """Writes the given desired positions.
        ...
        # Convert to Dynamixel position space.
        vel = vel / self._pos_vel_cur_reader.vel_scale
        self.sync_write(motor_ids, vel, ADDR_profile_velocity,
                        LEN_GOAL_POSITION)

    def write_acc_profile(self, motor_ids: Sequence[int],
                          acc: np.ndarray):
        """Writes the given desired positions.
        ...
        Args:
            ...
            positions: The joint angles in radians to write.
        """
        assert len(motor_ids) == len(acc)

        # Convert to Dynamixel position space.
        acc = acc / self._pos_vel_cur_reader.cur_scale
        self.sync_write(motor_ids, acc, ADDR_profile_acc,
                        4)
